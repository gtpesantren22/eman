            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <?php if (isset($_POST['cek'])) {
                        ...
                        $pl = query("SELECT * FROM pelanggaran WHERE nis = '$nis' ");
                        $jp = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM pelanggaran WHERE nis = '$nis' "));
                    ...
                    <form action="" method="post">
                        ...
                        <span class="badge badge-secondary">jumlah pelanggaran : <?= $jp; ?> kali</span>
                        <hr>
                        <!-- Comment Row -->
                        <?php $i = 1; ?>
                        <?php foreach ($pl as $r) : ?>
                        <div class="comment-widgets scrollable">
                            <div class="d-flex flex-row comment-row m-t-0">
                                <div class="comment-text w-100">
                                    <h6 class="font-bold"><?= $r['kasus'] ?></h6>
                                    <span class="m-b-15 d-block"><?= $r['tempat']; ?>
                                        (<?= tanggal_indo($r['tanggal'], true); ?>) </span>
                                    <div class="comment-footer">
                                        <?php
                                                if ($r['stts'] == 'Sudah ditangani') {
                                                    echo "<label class='label label-success float-right'><span class='fa fa-check'></span> Sudah ditangani
                        </label>";
                                                } else {
                                                    echo "<label class='label label-danger float-right'><span class='fas fa-window-close'></span> Belum ditangani
                        </label>";
                                                }
                                                ?>
                                        <a
                                            href="<?= 'index.php?link=pages/pelanggaran/detail&nis=' . $r['nis'] . '&id=' . $r['id']; ?>"><button
                                                type="button" class="btn btn-cyan btn-sm"><span
                                                    class="fas fa-list-alt"></span> Detail</button></a>
                                    </div>
                                </div>
                            </div>
                        </div>
